...
const BudgetPage = () => {
    ...
    const userId = "662117b3b753f10b74f8dd2b";
        ...
        const fetchData = async () => {
            try {
                ...
                const playerDataRequests = playerListResponse.data.map(async (player) => {
                    const playerDataResponse = await axios.get(`/api/players/${player.playerId}`);
    ...
    const handleBuy = async (playerId, playerOverallRating) => {
        try {
            const response = await axios.get(`/api/user/${userId}/budget`);
            ...
            await axios.put(`/api/user/${userId}/budget`, { budget: newBudget });
            await axios.post('/api/userplayers', { userId, playerId });
    ...
    const handleSell = async (playerId, playerOverallRating) => {
        try {
            ...
            await axios.put(`/api/user/${userId}/budget`, { budget: newBudget });
            await axios.delete(`/api/userplayers/${userId}/${playerId}`);
